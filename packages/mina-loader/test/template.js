...
test('inline template', async (t) => {
  try {
    ...
    t.is(mfs.readFileSync('/fixtures/template/inline.json', 'utf8'), '{\n  "name": "mina"\n}')
...
test('import template', async (t) => {
  try {
    ...
    t.is(mfs.readFileSync('/wxml/message.fd0e28.wxml', 'utf8'), '<!--\n  index: int\n  msg: string\n  time: string\n-->\n<template name="message">\n  <view>\n    <text> {{index}}: {{msg}} </text>\n    <text> Time: {{time}} </text>\n  </view>\n</template>\n')

    t.true(mfs.readFileSync('/fixtures/template/import.js', 'utf8').includes('this is a template'))
    t.is(mfs.readFileSync('/fixtures/template/import.wxml', 'utf8'), '<view>\n  <import src="../../wxml/message.fd0e28.wxml"/>\n  <template is="message" data="{{...item}}"/>\n</view>')
    t.is(mfs.readFileSync('/fixtures/template/import.wxss', 'utf8'), 'view {\n  color: #00f;\n}')
    t.is(mfs.readFileSync('/fixtures/template/import.json', 'utf8'), '{\n  "name": "mina"\n}')
